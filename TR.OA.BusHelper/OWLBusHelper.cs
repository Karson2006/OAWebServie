    ...
    public class OWLBusHelper
    {
        ...
        public string SubmitEntertainmentExpendForm(string xmlString)
        {
            ...
            string formData = @"<formExport version=""2.0""""><summary id=""7810778149231805299"" name=""formmain_5935""/>
                                            ...
                                            <column name=""可用额""><value>{19}</value></column>
                                            <column name=""财务审批标志""><value>{20}</value></column>
                                            <column name=""费用来源""><value>{21}</value></column>
                                            <column name=""是否有发票""><value>0</value>
                                            </column><column name=""成本中心_归档""><value>{4}</value></column>
            ...
            try
            {
                ...
                sql = @"Select t1.ID As FEmployeeID, t1.Code As FEmployeeCode,t1.ORG_DEPARTMENT_ID AS FDeptID, Isnull(t2.field0008,'') As FCosstCenter,
                            ...
                            Left Join v3x.dbo.formmain_8130 t3 On Cast(t1.ID As varchar(25))= t2.field0001
                ...
                //读取招待费明细表可用额
                sql = @"Select t1.field0006 As field0045,t2.SHOWVALUE AS field0045_Name,t1.field0009 As field0036,t3.field0011 As field0017
                ...
                {
                    rowDatas = rowDatas + string.Format(rowData, row["field0039"].InnerText, row["field0040"].InnerText, row["field0041"].InnerText,
                        row["field0042"].InnerText, row["field0043"].InnerText, row["field0044"].InnerText);
                }
                formData = string.Format(formData, EmployeeeID, field0003, field0024, field0006, field0010, field0011, field0012, field0013, field0017, field0018, field0019,
                    field0020, field0021, field0022, field0023, field0025, field0031, field0035, field0036, -1, field0045, rowDatas);

                sql = @"Insert Into DataSevice.dbo.OATask([FTemplateCode],[FSenderLoginName],[FEmployeeCode],[FEmployeeName],[FSubject],[FData])
	                     Values('OWL_HX_2020_01','{0}','{1}','{2}','{3}' , '{4}')";
                sql = string.Format(sql, loginName, field0010, EmployeeeName, formData);
